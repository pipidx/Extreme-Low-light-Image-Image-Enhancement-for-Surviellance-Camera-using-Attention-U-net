...
gt_dir = './dataset/long/'
checkpoint_dir = './checkpoint/new_at_unet/'
result_dir = './result/'
...
out_image = model.AUnet_network(in_image, 3)
...
for epoch in range(lastepoch,4001):
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        if input_images[str(ratio)[0:3]][ind] is None:     
            ...
            input_images[str(ratio)[0:3]][ind] = np.expand_dims(np.float32(in_img/65535.0),axis = 0) * ratio

            gt_img = tifffile.imread(gt_path)
            gt_images[ind] = np.expand_dims(np.float32(gt_img/65535.0),axis = 0)
        ...
        if epoch%save_freq==0:
            ...
            scipy.misc.toimage(temp*255, high=255, low=0, cmin=0, cmax=255).save(result_dir + '%04d__%05d_00_train_%d.jpg'%(epoch,train_id,ratio))
